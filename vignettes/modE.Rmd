...
```{r hotspot-plot}
KanPlot(cosmx, feature = 'CD74_clust', palette_discrete = c('dodgerblue3','firebrick3','gray85'))
```


## 4) Loading cell type annotation
...
(KanPlot(cosmx, feature = 'final_anno', fovs = fovs
         , show_border = T, border.size = 0.04, border.col = 'gray70') +
    scale_fill_manual(values = c('Epi' = 'green', 'FibroEndoMuscle' = 'magenta', 'Myeloid' = 'cyan', 'T/NK' = 'red', 'Plasma/B' = 'yellow'))) +
  
  KanPlot(cosmx, feature = 'CD74', fovs = fovs
          , palette_cont = 'viridis', show_border = T, border.size = 0.04, border.col = 'gray70') +
  
  KanPlot(cosmx,feature = 'CD74_clust', fovs = fovs
          , palette_discrete = c('dodgerblue3','firebrick3','gray85'), show_border = T, border.size = 0.04, border.col = 'gray70')
```
...
for(c in c('Epi','Myeloid','TNK')){
  ...
  cosmx_hot[[c]] = ggplot(hot_props, aes(x = Var1, y = prop))+
    theme_classic() + geom_bar(stat = 'identity', fill = 'firebrick3', color = 'black') +
    scale_x_discrete(breaks = c('1','0'), limits = c('1','0'), labels = c('Hot','Rest')) +
    labs(x = 'CD74 areas', y = paste0('Cells (%)')) +
...
ggplot(cosmx_cd74_data, aes(x = CD74_expr, fill = Epi_type)) +
  ...
  scale_fill_manual(values = c('seagreen4','green')
                    , labels = c(paste0('Epi (', n_epi1, ')'), paste0('Epi near T/NK (', n_epi2, ')'))) +
  geom_vline(xintercept = median1, linetype = 'dashed', color = 'seagreen4') +
  geom_vline(xintercept = median2, linetype = 'dashed', color = 'green')
```
...
visium = hotspot_analysis(visium, feature='CD74', sim = 1999, padj.thresh = 0.05)

KanPlot(visium, feature = 'CD74_clust', palette_discrete = c('dodgerblue3','firebrick3','gray85'))
```
...
signatures = signatures[c(4,5,6,10,12,13,14,15)]
...
for(s in paste0(names(signatures),'_UCell_kNN')){
  visium = hotspot_analysis(visium, feature = s, sim = 1999, padj.thres = 0.05)
}
...
KanPlot(visium, feature = 'M1_16_clust', palette_discrete = c('dodgerblue3','firebrick3','gray85'))
...
for(s in paste0(names(signatures),'_clust')){
  ...
  plots_cold[[s]] = ggplot(cold_props,aes(x = Var1, y = prop)) + 
    theme_classic() + geom_bar(stat = 'identity', fill = 'dodgerblue3', color = 'black')+
